...
class AdServer:
    ...
    def __init__(
        self, database_host, database_username, database_password, database_name
    ):
    ...
    def fetchAds(self):
        ...
        sql_fetch_auctionCost += (
            conditionStatement
            + " order by cpm desc limit 2 ) as temp order by cpm limit 1;"
        )

        # Gathering details from the Winner AD
        sql_fetch_auctionWin += (
            conditionStatement + " order by cpm desc limit 2) as temp limit 1;"
        )
    ...
    def Served_entry(self):
        ...
        if len(self.adDetails) != 0 and len(self.DecidedAuctionCost[0]) != 0:
            ...
            startDateTime = datetime.datetime.combine(
                self.adDetails[0][18],
                (datetime.datetime.min + self.adDetails[0][20]).time(),
            )
            endDateTime = datetime.datetime.combine(
                self.adDetails[0][19],
                (datetime.datetime.min + self.adDetails[0][21]).time(),
            )

            # SQL insert Operation Statement
            sql = (
                "INSERT INTO served_ads("
                "requestID ,"
                "campaignID ,"
                "userID ,"
                "auctionCPM ,"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 5:
        print(
            "Usage: <database_host> <database_username> <database_password> <database_name>"
        )
        exit(-1)
    ...
    try:
        ...
        def serve(user_id):
            ...
            if len(ad_server_object.adDetails) != 0:
                return jsonify(
                    {"request_id": request_id, "text": ad_server_object.adDetails[0][0]}
                )
