...
class KafkaMySQLSink:
    def __init__(
        ...
        self.consumer = kafka_client.topics[kafka_topic_name].get_simple_consumer(
            consumer_group="campaign_id", auto_offset_reset=OffsetType.LATEST
        )
    ...
    def process_events(self):
        def dervied_attribute(adInfo):
            def SlotBudgetCalculation(budget, dateperiod, timeperiod):
                Slotslist = []
                start = dt.strptime(
                    dateperiod["start"] + " " + timeperiod["start"], "%Y-%m-%d %H:%M:%S"
                )
                end = dt.strptime(
                    dateperiod["end"] + " " + timeperiod["end"], "%Y-%m-%d %H:%M:%S"
                )
                while start <= end:
            ...
            adInfo["status"] = (
                "INACTIVE" if adInfo["action"] == "Stop Campaign" else "ACTIVE"
            )
            adInfo["cpm"] = 0.0075 * float(adInfo["cpc"]) + 0.0005 * float(
                adInfo["cpa"]
            )
            adInfo["current_slot_budget"] = SlotBudgetCalculation(
                adInfo["budget"], adInfo["date_range"], adInfo["time_range"]
            )
            return adInfo
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 7:
        print(
            "Usage: ad_manager.py <kafka_bootstrap_server> <kafka_topic> "
    ...
    try:
        kafka_mysql_sink = KafkaMySQLSink(
            kafka_bootstrap_server,
            kafka_topic,
            database_host,
            database_username,
